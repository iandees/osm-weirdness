...
def minutelyUpdateRun():
  ...
  url = "http://planet.openstreetmap.org/hour-replicate/%s/%s/%s.osc.gz" % (sqnStr[0:3], sqnStr[3:6], sqnStr[6:9])
  ...
  # Look for weird nodes
  for node in handler.nodes.itervalues():
    if node['action'] == 'delete':
      # Skip deleted nodes for now
      continue

    if node['lat'] == 0.0 and node['lon'] == 0.0:
      print "!!! Node %d sits at Null Island." % (node['id'],)
  ...
  url = "http://planet.openstreetmap.org/hour-replicate/%s/%s/%s.state.txt" % (sqnStr[0:3], sqnStr[3:6], sqnStr[6:9])
  try:
